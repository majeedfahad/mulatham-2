...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 animate-fade-in">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            @if($player->is_host)
                <form action="{{ route('game.end', $room->code) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من إنهاء اللعبة؟')">
                    @csrf
    ...
    <div class="mb-4 animate-fade-in">
        ...
        <div class="game-progress">
            <div class="game-progress-bar" style="width: {{ $totalQuestions > 0 ? ($currentQuestionNumber / $totalQuestions) * 100 : 0 }}%"></div>
        </div>
    </div>
    @endif
    ...
    @if(session('reveal_result'))
        ...
        <div class="game-alert {{ $result['is_correct'] ? 'game-alert-success' : 'game-alert-danger' }} mb-4 animate-fade-in">
            ...
            @else
                ...
                <div>
                    ...
                    <span>{{ $result['target_name'] }} ليس {{ $result['guessed_name'] }} - تم نقل نقاطك وأنت الآن خارج اللعبة</span>
                </div>
            @endif
        </div>
    @endif
                    ...
                    <div class="game-card-body">
                        ...
                        <div class="question-form-container mb-4">
                            ...
                            <form id="addQuestionForm">
                                    ...
                                    <div class="choices-container">
                                        ...
                                        <div class="choice-input-group mb-2">
                                            <label class="choice-option d-flex align-items-center gap-2 w-100">
                                                <input type="radio" name="correct_choice_index" value="{{ $i }}" class="choice-radio" {{ $i === 0 ? 'checked' : '' }}>
                                                <input
                                                    type="text"
                                                    name="choices[]"
                                                    class="game-input flex-fill mb-0"
                                ...
                                <button type="submit" class="btn-game btn-game-primary w-100 btn-game-lg mt-3" id="addQuestionBtn">
                        ...
                        <div class="my-questions-section">
                            ...
                            <div class="questions-list" id="playerQuestionsList">
                                ...
                                <div class="question-item d-flex align-items-center justify-content-between p-3 mb-2" data-question-id="{{ $q->id }}">
                                    <div class="flex-fill">
                                    ...
                                    <button type="button" class="btn-game btn-game-outline btn-game-sm delete-question-btn" data-question-id="{{ $q->id }}">
                ...
                <div class="row g-3 mb-4" id="answersGrid">
                    @foreach($answers as $answer)
                        <div class="col-md-4 col-sm-6">
                            <div class="answer-card {{ $answer['is_revealed'] ? 'border-info' : '' }} {{ $answer['is_eliminated'] ? 'opacity-50' : '' }} {{ $answer['is_correct'] ? 'answer-correct' : 'answer-wrong' }}">
                                <div class="fake-name">
                                ...
                                @if($player->canReveal() && !$answer['is_revealed'] && !$answer['is_eliminated'] && $answer['player_id'] !== $player->id)
                                    <button
                                        type="button"
                                        class="btn-game btn-game-danger btn-game-sm w-100 reveal-btn"
                                        data-target-id="{{ $answer['player_id'] }}"
                                        data-target-name="{{ $answer['fake_name'] }}"
                                    >
...
<!-- Reveal Modal -->
<div class="modal fade" id="revealModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="false" data-bs-keyboard="false">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex flex-column gap-2" id="playersList">
                        @foreach($realPlayers as $rp)
                            @if($rp->id !== $player->id)
                                <button type="button" class="player-card player-select-card d-flex align-items-center gap-3 w-100 text-start" data-player-id="{{ $rp->id }}">
                                    <div class="player-avatar" style="width: 40px; height: 40px; font-size: 1rem;">
                                        {{ mb_substr($rp->name, 0, 1) }}
                                    </div>
                                    <span class="fw-bold">{{ $rp->name }}</span>
                                    <i class="bi bi-check-circle-fill text-success ms-auto check-icon" style="display: none;"></i>
                                </button>
                            @endif
                        @endforeach
...
<form action="{{ route('game.revealTimeout', $room->code) }}" method="POST" id="revealTimeoutForm" style="display: none;">
    @csrf
</form>
                ...
                .listen('.game.state.updated', (e) => {
                    ...
                    switch (e.type) {
                        ...
                        case 'answer_submitted':
                            console.log('Answer count:', e.data.answers_count);
                            break;
